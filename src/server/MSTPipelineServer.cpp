...
void MSTPipelineServer::start() {
    setupSocket();
    struct sockaddr_storage client_addr; // client address
    socklen_t addrlen;
    std::cout << "Pipeline MST server started on port " << PORT << std::endl;
    ...
    while (1) {
        ...
        std::thread client_thread(&MSTPipelineServer::handleClient, this, client_data);
        client_thread.detach(); // Detach the thread so it can run independently
    }
}

void MSTPipelineServer::stop() {
...
void MSTPipelineServer::handleClient(ClientData *client_data) {
...
void MSTPipelineServer::setupSocket() {
...
void MSTPipelineServer::processCommand(int socket_fd, std::string &command) {
